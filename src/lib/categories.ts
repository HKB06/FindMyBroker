/**
 * Définition des catégories d’articles / brokers
 * -------------------------------------------------
 *  • `CategorySlug` : union littérale pour l’auto-complétion
 *  • `Category`     : interface réutilisable dans le front
 *  • `categories`   : tableau typé exporté
 */
...
export interface Category {
  slug: CategorySlug;
  name: string;
  color: string;   // classes Tailwind (bg-*)
  desc: string;    // description longue
  pres: string;    // forme courte pour les phrases (“la bourse”, “le trading”…)
  emoji: string;
}
